...
async function startProcess(){if(!currentResult||!currentResult.data||!currentResult.data.url){showError("请先解析视频")return}processModal.hidden=falseprocessDone.hidden=trueprocessBar.style.background=""processBar.style.width="0"processTitle.innerHTML="<span class=\"zh\">视频指纹处理中...</span><span class=\"en\">Processing...</span>"processBtn.disabled=trueprocessBar.style.width="50%"const ctrl=new AbortController()setTimeout(()=>ctrl.abort(),120000)try{const fd=new FormData()fd.append("url",currentResult.data.url)fd.append("filename",currentResult.data.title||"video")const r=await fetch("api/v1/process",{method:/POST/,body:fd,signal:ctrl.signal}),j=await r.json().catch(()=>null)processBar.style.width="100%"if(!r.ok||!j||!j.success)throw new Error((j&&j.message)||"处理失败")processTitle.innerHTML="<span class=\"zh\">视频指纹处理完成</span><span class=\"en\">Fingerprint Complete</span>"processDone.hidden=falseprocessDlBtn.dataset.dlUrl=j.data.urlprocessDlBtn.dataset.dlFilename=j.data.filenameprocessMd5.textContent="MD5: "+j.data.md5.slice(0,16)+"  |  "+Math.round(j.data.size/1024/1024)+"MB"processBtn.disabled=false}catch(e){processBar.style.width="100%"processBar.style.background="#e64555"processTitle.innerHTML=e.name==="AbortError"?"<span class=zh>处理超时</span><span class=en>Timed out</span>":"<span class=zh>视频指纹处理失败，请重新处理</span><span class=en>Processing failed, please retry</span>"processBtn.disabled=false}}
