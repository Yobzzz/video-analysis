...
<div id="process-modal" class="process-modal" hidden><div class="process-modal-bg"></div><div class="process-modal-box"><h3 id="process-title"><span class="zh">视频指纹处理中</span><span class="en">Processing Video</span></h3><div class="process-bar-wrap"><div class="process-bar"></div></div><div id="process-done" class="process-done" hidden><a id="process-dl-btn" class="btn btn-primary" download><span class="zh">下载处理后视频</span><span class="en">Download Processed</span></a><p class="process-md5" id="process-md5"></p></div><button id="process-close" class="process-close-btn" type="button"><span class="zh">关闭</span><span class="en">Close</span></button></div></div>
...
function mediaUrl(url){return'index.php?action=media&url='+encodeURIComponent(url)}function buildDownloadUrl(url,title){return'dl/'+encodeURIComponent((title||'video').slice(0,60).replace(/[<>:"/\\|?*]/g,'')||'video')+'.mp4?url='+encodeURIComponent(url)}
...
async function startProcess(){if(!currentResult||!currentResult.data||!currentResult.data.url){showError('请先解析视频');return}processModal.hidden=false;processDone.hidden=true;processBar.style.background='';processBar.style.width='0';processTitle.innerHTML='<span class=\"zh\">视频指纹处理中...</span><span class=\"en\">Processing...</span>';processBtn.disabled=true;processBar.style.width='30%';try{const fd=new FormData();fd.append('url',currentResult.data.url);fd.append('filename',currentResult.data.title||'video');processBar.style.width='60%';const r=await fetch('api/v1/process',{method:'POST',body:fd}),j=await r.json().catch(()=>null);if(!r.ok||!j||!j.success)throw new Error((j&&j.message)||'处理失败');processBar.style.width='100%';processTitle.innerHTML='<span class=\"zh\">视频指纹处理完成</span><span class=\"en\">Fingerprint Complete</span>';processDone.hidden=false;processDlBtn.href=j.data.url;processDlBtn.setAttribute('download',j.data.filename);processMd5.textContent='MD5: '+j.data.md5.slice(0,16)+'  |  '+Math.round(j.data.size/1024/1024)+'MB';processBtn.disabled=false}catch(e){processBar.style.width='100%';processBar.style.background='#e64555';processTitle.innerHTML='<span class=zh>视频指纹处理失败，请重新处理</span><span class=en>Processing failed, please retry</span>';processBtn.disabled=false}}
processClose.addEventListener('click',()=>{processModal.hidden=true;processBar.style.width='0';processBar.style.background='';processDone.hidden=true})
