...
parseBtn.addEventListener('click',parseVideo);input.addEventListener('keydown',e=>{if(e.key==='Enter')parseVideo()});pasteBtn.addEventListener('click',async()=>{try{const text=await navigator.clipboard.readText();input.value=extractUrl(text);input.focus()}catch(e){showError('无法读取剪贴板，请手动粘贴')}});copyBtn.addEventListener('click',async()=>{if(currentResult&&downloadBtn.href){await copyText(downloadBtn.href);flashButton(copyBtn,'已复制')}});copyTextBtn.addEventListener('click',async()=>{if(currentResult){await copyText(buildShareText(currentResult));flashButton(copyTextBtn,'已复制')}});video.addEventListener('error',()=>{if(!ignoreMediaError){video.style.opacity='0.3';let p=video.parentElement.querySelector('.video-fallback');if(!p){p=document.createElement('p');p.className='video-fallback';p.innerHTML='<span class=zh>预览不可用，请点下载按钮</span><span class=en>Preview unavailable, use download</span>';video.parentElement.appendChild(p)}}})
